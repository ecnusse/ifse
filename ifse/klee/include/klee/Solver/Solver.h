...
namespace klee {
  ...
  std::unique_ptr<Solver> createKRPKCexCachingSolver(std::unique_ptr<Solver> s);
  ...
  /// createFilteringSolver - Create a wrap solver which could filter the 
  /// query, according to the Colossus algorithm
  /// \param s - The underlying solver to use.
  std::unique_ptr<Solver> createFilteringSolver(std::unique_ptr<Solver> s);

  /// createWrapKRPKSolver - Create a wrap KRPK solver which could forward 
  /// methods
  /// \param s - The underlying solver to use.
  std::unique_ptr<Solver> createWrapKRPKSolver(std::shared_ptr<Solver> solver);
