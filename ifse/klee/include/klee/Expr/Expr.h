...
using SymcreteMap = std::map<std::string, std::pair<const klee::Array*, std::vector<uint8_t>>>;
using SymcreteMapPtr = std::shared_ptr<SymcreteMap>;

void setSymcreteMapSingleton(klee::SymcreteMapPtr symcreteMap);

void clearSymcreteMapSingleton();

klee::SymcreteMapPtr getSymcreteMapSingleton();
...
public:
  ...
  ref<Expr> index, value; //index represents the index of the byte which is updated in array
...
class SymbolicExpr : public NonConstantExpr {
public:
  const Array* array;
  static const Kind kind = Symbolic;
protected:
  int compareContents(const Expr &b) const { message(); return 0; }
public:
  SymbolicExpr(const Array* _array) : array(_array) {}
  Kind getKind() const { return Symbolic; }
  static void message() {
    assert(0 && "Unsupported interface:"
          "Symbolic is not blessed to be enjoyed because of its short life.");
  }
  Width getWidth() const { message(); return 0; }
  // unsigned getNumKids() { message(); return 0; }
  ref<Expr> getKid(unsigned i) const { message(); return 0; }
  unsigned getNumKids() const { message(); return 0; }
  ref<Expr> rebuild(ref<Expr> kids[/* getNumKids() */]) const { message(); return 0; }
};
...
public:
  ...
  //create a ReadExpr representing the Expr read from UpdateNode with index from ul.
  //index represents the index of byte which is updated in the updatelist
...
private:
  ReadExpr(const UpdateList &_updates, const ref<Expr> &_index) : 
    updates(_updates), index(_index) { 
      assert(updates.root); 
      //_index->print(llvm::errs());
      // assert(_index->getKind() == Constant); 
    }
...
class ExtOpExpr : public Expr {
public:
  static const Kind kind = ExtOp;

private:
  const KFunction* func;
  const std::vector<ref<Expr>> args;

  ExtOpExpr(const KFunction* _func, const std::vector<ref<Expr>>& _args) 
    : func(_func), args(_args) { }

public:
  static ref<ExtOpExpr> create(const KFunction* func, const std::vector<ref<Expr>>& args) {
    ref<ExtOpExpr> r(new ExtOpExpr(func, args));
    return r;
  }
  
  virtual ref<Expr> rebuild(ref<Expr> kids[]) const {
    std::vector<ref<Expr>> args = std::vector<ref<Expr>>(this->args.size());
    for (unsigned i = 0; i < args.size(); i++) {
      args[i] = kids[i];
    }
    auto r = ExtOpExpr::create(func, args);
    return r;
  }
  ...
  Width getWidth() const { 
    llvm::Type* returnType = func->function->getReturnType();
    if(returnType->isIntegerTy()) {
      ...
      switch (width) {
        ...
        case 64:
          return Expr::Int64;
        default:
          assert(0 && "width in isIntegerTy of extop to be handled\n");
      }
    } else if(returnType->isPointerTy()) {
      return Expr::Int64;
    } else {
      assert(0 && "width in isPointerTy of extop to be handled\n");
    }
  }
  Kind getKind() const { 
    return kind;
  ...
  // FIXME: Consider pointer args
  int compareContents(const Expr &b) const {
    const ExtOpExpr &eob = static_cast<const ExtOpExpr &>(b);
    if (func != eob.func)
      return func < eob.func ? -1 : 1;
    if (args != eob.args)
      return args < eob.args ? -1 : 1;
    return 0;
  }

  unsigned computeHash();

  const KFunction* getFunc() const { return func; }
