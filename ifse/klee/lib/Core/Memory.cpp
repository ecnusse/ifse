...
const UpdateList &ObjectState::getUpdates() const {
  // if we want to get updates from a objectstate which not bind to some array
  // Then create a constant array as the root of updates
  if (!updates.root) {
    ...
    //the larger index is, the newer write is.
    ...
    // if index or value is not ConstantExpr, then break.
    ...
    //using constant arrays for non-symbolic objects
    static unsigned id = 0;
...
// With the introduction of ‘symcrete’ functionality, 
// This function not only flushes constant to concreteStore but also converts solver assignment results into ‘symcrete’ form.
void ObjectState::flushToConcreteStore(TimingSolver *solver,
                                        const ExecutionState &state, bool concretize, bool isExtOp) const {
  if (RecolossusDebugLog) { klee_warning("DEBUG: Enter flushToConcreteStore");}
  for (unsigned i = 0; i < size; i++) {
    if (isByteConcrete(i))
      continue;
    // Get a concrete value for the symbolic byte and write it to the memory object
    auto resultPair = solver->getValueAndSolution(state.constraints, read8(i), state.queryMetaData);
    if (!resultPair.hasValue()) {
      ...
      if (isExtOp) {
        auto assignment = resultPair->first;
        // bindAssignment
        for (auto& b: assignment.bindings) {
          state.addSymcreteMap(b.first, b.second);
        }
      }
      // flush to concrete store
      ref<ConstantExpr> ce = resultPair->second;
      ce->toMemory(concreteStore + i);
    }
  }
  if (RecolossusDebugLog) { klee_warning("DEBUG: Leave flushToConcreteStore");}
}
...
void ObjectState::makeSymbolic() {
  ...
  for (unsigned i=0; i<size; i++) {
    markByteSymbolic(i);
    setKnownSymbolic(i, 0); //makeSymbolic() would not allocate space for knownSymbolics because value is set to 0.
    markByteFlushed(i);
  }
}
...
void ObjectState::fastRangeCheckOffset(ref<Expr> offset,
 ...
 std::vector<ref<Expr>> ObjectState::getAllBytes() const {
...
void ObjectState::flushRangeForWrite(unsigned rangeBase, unsigned rangeSize) {
  ...
  for (unsigned offset = rangeBase; offset < rangeBase + rangeSize; offset++) {
    if (isByteUnflushed(offset)) {
      if (isByteConcrete(offset)) {
        //flush concrete byte into updatelist, then mark the byte symbolic for further writing
        updates.extend(ConstantExpr::create(offset, Expr::Int32),
                       ConstantExpr::create(concreteStore[offset], Expr::Int8));
        ...
        assert(isByteKnownSymbolic(offset) &&
               "invalid bit set in unflushedMask");
        //flush symbolic byte into updatelist, then set the symbolic byte to 0(initial value) for further writing
        updates.extend(ConstantExpr::create(offset, Expr::Int32),
                       knownSymbolics[offset]);
...
ref<Expr> ObjectState::read8(unsigned offset) const {
  if (isByteConcrete(offset)) {
    ...
    return knownSymbolics[offset];
     ...
     //the byte is not concrete but knownSymbolics has not been allocated, so create a readExpr from updateList.
...
ref<Expr> ObjectState::read8(ref<Expr> offset) const {
  ...
  flushRangeForRead(base, size); //flush cache to updatelist (record current cache state) before symbolic read.
  ...
  return ReadExpr::create(getUpdates(), ZExtExpr::create(offset, Expr::Int32)); //symbolic read, not actual read
}
...
void ObjectState::write8(unsigned offset, ref<Expr> value) {
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(value)) {
    write8(offset, (uint8_t) CE->getZExtValue(8));
     ...
     //write symbolic expr into knownSymbolics and mark byte unflushed.
...
void ObjectState::write8(ref<Expr> offset, ref<Expr> value) {
  ...
  updates.extend(ZExtExpr::create(offset, Expr::Int32), value); //symbolic write, not actual write
}
...
void ObjectState::print() const {
  ...
  if(updates.root) {
    llvm::errs() << "\tRoot Array: " << updates.root->name << "\n";
  } else {
    llvm::errs() << "\tRoot Array: " << "NULL" << "\n";
  }
  llvm::errs() << "\tSize: " << size << "\n";
  llvm::errs() << "\tBytes:\n";
  for (unsigned i=0; i<size; i++) {
