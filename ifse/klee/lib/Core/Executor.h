...
private:
  ...
  std::unique_ptr<FuzzingSolver> fuzzingSolver;
  ...
  ref<Expr> makeFreshSymcrete(ExecutionState& state, ref<ConstantExpr> ce, KInstruction* target);

  // When using REColossus, also set symcretes according to the algorithm.
  ...
  void executeCall(ExecutionState &state, KInstruction *ki,
  ...
  /// When using symcretes, conduct extra fuzz solving when only one 
  /// branch proved to be feasible by the traditional SMT solver.
  StatePair fork(ExecutionState &current, ref<Expr> condition, bool isInternal,
                 BranchType reason, std::string locInfo = "NO_DEBUG_INFO");
  ...
  // Get operand of KInstruction ki by index
  ...
  // Make argument pass by registers symcrete. Arguments in memory are set in ObjectState::flushToConcreteStore.
  std::vector<ref<Expr>> specialHandleForPointerInArgs(llvm::Function* func, ExecutionState& state, std::vector<Cell*> argCells, std::vector<ref<Expr>> argValues);

  // Add an extop related constraint.
  void addExtOpConstraint(KInstruction* ki, ExecutionState& state, ref<Expr> returnSymcretes, std::vector<ref<Expr>> argSymcretes);

  // Bind arrays to their symcrete values.
  void bindAssignment(Assignment& assignment, ExecutionState& state);
...
public:
  ...
  bool isExtOp(const KInstruction *ki, const llvm::Function* f);
  bool checkKeyExists(const std::string& str, const std::string& key);

  void setSymcreteMap(const ExecutionState &state);
