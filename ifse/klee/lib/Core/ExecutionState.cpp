...
ExecutionState::ExecutionState(KFunction *kf, MemoryManager *mm)
  ...
  symcreteMap = std::make_shared<std::map<std::string, ExecutionState::bindings_ty>>();
}
...
ExecutionState::ExecutionState(const ExecutionState& state):
    ...
    forkDisabled(state.forkDisabled) {
      ...
      symcreteMap = std::make_shared<std::map<std::string, bindings_ty>>(*state.getSymcreteMap());
    }
...
void ExecutionState::addConstraint(ref<Expr> e) {
  setSymcreteMapSingleton(this->getSymcreteMap());
...
llvm::Optional<std::vector<unsigned char>> ExecutionState::findSymcreteMap(const Array *array) const {
  if (!useRecolossus) {
    return llvm::None;
  }
  assert(symcreteMap && "FIXME: Error in findSymcreteMap");
  auto it = symcreteMap->find(array->name);
  if (it == symcreteMap->end()) {
    return llvm::None;
  }
  return it->second.second;
}

void ExecutionState::addSymcreteMap(const Array *array, std::vector<unsigned char> concrete) const {
  if (!useRecolossus) {
    return;
  }
  assert(symcreteMap && "FIXME: Error in addSymcreteMap");
  // symcreteMap.insert_or_assign(array->name, std::make_pair(array, concrete));
  // Unfortunately, it is only supported in C++17
  auto it = symcreteMap->find(array->name);
  if (it != symcreteMap->end()) {
    symcreteMap->erase(it);
  }
  symcreteMap->emplace(array->name, std::make_pair(array, concrete));
}

std::shared_ptr<std::map<std::string, ExecutionState::bindings_ty>> ExecutionState::getSymcreteMap() const {
  return symcreteMap;
}

llvm::Optional<std::pair<ref<const MemoryObject>, const Array *>> ExecutionState::getArrayFromSymbolic(const std::string& name) const {
  for(auto &tmp : symbolics) {
    if(tmp.second->getName() == name) {
      return tmp;
    }
  }
  return llvm::None;
...
bool ExecutionState::hasExtOp() const {
  for (auto c : constraints) {
    if (c->getKind() == Expr::Eq) {
      auto eqExpr = cast<EqExpr>(c);
      if (eqExpr->right->getKind() == Expr::ExtOp) {
        return true;
      } 
    }
  }
