...
cl::opt<bool> useRecolossus("recolossus",
          cl::desc("Use Colossus algorithm (default=false)"),
          cl::init(false));

cl::opt<std::string> RecolossusRange(
    "recolossus-range",
    cl::desc("Apply Colossus algorithm over a specified range."
                 "(default=,)"),
    cl::init(","),
    cl::cat(TerminationCat));

cl::opt<std::string> RecolossusExternalFunction(
    "recolossus-external-function",
    cl::desc("Apply Colossus algorithm to a specified external function."
                "(default=,)"),
    cl::init(","),
    cl::cat(TerminationCat));

cl::opt<std::string> RecolossusLine(
    "recolossus-line",
    cl::desc("Apply Colossus algorithm to an external function called at the given line."
                "(default=0)"),
    cl::init("0"),
    cl::cat(TerminationCat));

cl::opt<bool> RecolossusDebugLog(
    "recolossus-debug-log",
    cl::desc("Apply Colossus algorithm to an external function called at the given line."
                "(default=false)"),
    cl::init(false),
    cl::cat(TerminationCat));
...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
  ...
  fuzzSolverTimeout = time::Span{MaxFuzzSolverTime};
  ...
  if (useRecolossus)
    solver = createFilteringSolver(std::move(solver));
  this->solver = std::make_unique<TimingSolver>(std::move(solver), EqualitySubstitution);

  if (useRecolossus) {
    std::shared_ptr<Solver> coreFuzzingSolver = std::make_shared<KRPKSolver>();
    if (!coreFuzzingSolver) {
      klee_error("Failed to create core fuzzing solver\n");
    }
    // FuzzingSolver: a simple wrapper solver invoking KRPKSolver
    this->fuzzingSolver = std::make_unique<FuzzingSolver>(coreFuzzingSolver);
    auto wrapCoreFuzzingSolver = createWrapKRPKSolver(coreFuzzingSolver);

    solver = createKRPKCexCachingSolver(std::move(wrapCoreFuzzingSolver));

    this->fuzzingSolver->setSolver(std::move(solver));
    assert(fuzzingSolver != nullptr && "fuzzingSolver is null");
  }
...
void Executor::setSymcreteMap(const ExecutionState &state) {
  if (useRecolossus) {
    FilteringSolver* filteringSolver = (FilteringSolver*)(solver->solver->impl.get());
    if (filteringSolver) { filteringSolver->setSymcreteMap(state.getSymcreteMap()); } 
    else {
      assert(0 && "Setting state error");
    }
  }
}

void Executor::clearSymcreteMap() {
  if (useRecolossus) {
    FilteringSolver* filteringSolver = (FilteringSolver*)(solver->solver->impl.get());
    if (filteringSolver) { filteringSolver->clearSymcreteMap(); } 
    else {
      assert(0 && "Clear symcreteMap error");
    }
  }
}

Executor::StatePair Executor::fork(ExecutionState &current, ref<Expr> condition,
                                   bool isInternal, BranchType reason, std::string locInfo) {
  if (RecolossusDebugLog) { klee_warning("DEBUG: Enter fork");}
  ...
  bool success = solver->evaluate(current.constraints, 
                                  condition, 
  ...
  if (res == Solver::True) {
    if (RecolossusDebugLog) { klee_warning("    - DEBUG: Validity is True");}
    if (!isSeeding && useRecolossus && current.hasExtOp() &&
        (reason == BranchType::Conditional || reason == BranchType::Switch || reason == BranchType::Indirect)) {
      // TODO: [REColossus-comment] line 17-20
      ref<Expr> nege = Expr::createIsZero(condition);
      fuzzingSolver->setTimeout(fuzzSolverTimeout);
      fuzzingSolver->setAddressSpace(&current.addressSpace);
      auto assignment = 
        fuzzingSolver->getFeasibleAssignment( current.constraints, nege, locInfo);

      // TODO: [REColossus] Copy out results in the memory.
      if (assignment.hasValue()) {
        ...
        for (auto b : assignment.getValue().bindings) {
            if (falseState->findSymcreteMap(b.first).hasValue())
              falseState->addSymcreteMap(b.first, b.second);
        }

        addConstraint(*trueState, condition);
        addConstraint(*falseState, nege);
    ...
    if (!isInternal) {
      if (pathWriter) { current.pathOS << "1"; }
    }
    if (useRecolossus) {
      addConstraint(current, condition);
    }
    ...
    if (RecolossusDebugLog) { klee_warning("    - DEBUG: Validity is False"); }
    if (!isSeeding && useRecolossus && current.hasExtOp() &&
        (reason == BranchType::Conditional || reason == BranchType::Switch || reason == BranchType::Indirect)) {
      // TODO: [REColossus-comment] line 23-26
      fuzzingSolver->setTimeout(fuzzSolverTimeout);
      fuzzingSolver->setAddressSpace(&current.addressSpace);
      auto assignment = 
        fuzzingSolver->getFeasibleAssignment(current.constraints, condition, locInfo);
      ...
      if (assignment.hasValue()) {
        ...
        processTree->attach(current.ptreeNode, trueState, falseState, reason);
        stats::incBranchStat(reason, 1);
        ...
        for (auto b : assignment.getValue().bindings) {
          if (trueState->findSymcreteMap(b.first).hasValue())
            trueState->addSymcreteMap(b.first, b.second);
        }
        ...
        // Kinda gross, do we even really still want this option?
        if (MaxDepth && MaxDepth<=falseState->depth) {
          terminateStateEarly(*falseState, "max-depth exceeded.", StateTerminationType::MaxDepth);
          terminateStateEarly(*trueState, "max-depth exceeded.", StateTerminationType::MaxDepth);
          return StatePair(nullptr, nullptr);
        }
    ...
    if (!isInternal) {
      if (pathWriter) { current.pathOS << "0"; }
    }
    if (useRecolossus) {
      addConstraint(current, Expr::createIsZero(condition));
    }
    ...
    if (RecolossusDebugLog) { klee_warning("    - DEBUG: Validity is neither True nor False"); }
    TimerStatIncrementer timer(stats::forkTime);
    ExecutionState *falseState, *trueState = &current;
...
ref<Expr> Executor::makeFreshSymcrete(ExecutionState& state, ref<ConstantExpr> ce, KInstruction* target) {
  // 1. Register a new colossus id
  static unsigned idColossus;
  std::string name = "colossus_fresh_var_arr" + llvm::utostr(++idColossus);

  // 2. Make a new symbolic variable, res is a read expr reading from array
  uint64_t size = Expr::getMinBytesForWidth(ce->getWidth());
  const Array *array = arrayCache.CreateArray(name, size);
  ref<Expr> res = Expr::createTempRead(array, ce->getWidth()); 

  // 3. Record symcrete info in global map
  auto value = ce->getZExtValue();
  std::vector<unsigned char> concrete;
  unsigned char* ptr = (unsigned char*)&value;

  #define BYTE_WIDTH 8
  for (unsigned i = 0; i < ce->getWidth() / BYTE_WIDTH; i++) {
    concrete.push_back(*(unsigned char*)(ptr + i));
  }
  state.addSymcreteMap(array, concrete);

  if (RecolossusDebugLog) {
    CallBase &cb = cast<CallBase>(*target->inst);
    Value *fp = cb.getCalledOperand();
    Function *f = getTargetFunction(fp); 
    klee_warning("DEBUG: makeFreshSymcrete: %s is returned by %s (%s %u)", 
                name.c_str(), f->getName().data(),target->info->file.c_str(), target->info->line);
  }
  // 4. Return symbolic (accually symcrete)
  return res;
}

void Executor::bindAssignment(Assignment& assignment, ExecutionState& state) {
  for (auto& b: assignment.bindings) {
    state.addSymcreteMap(b.first, b.second);
  }
}
...
ref<klee::ConstantExpr> Executor::toConstant (ExecutionState &state,
  ...
  // 2. Maybe kind of Symcrete 
  SymcreteReplacer replacer(&state);
  e = replacer.visit(e);

  // 3. Do not hold concrete, thus ask solver
  ref<ConstantExpr> value;
  bool success = solver->getValue(state.constraints, e, value, state.queryMetaData);
  assert(success && "FIXME: Unhandled solver failure");
  (void) success;
  ...
  if (AllExternalWarnings) { klee_warning("%s", os.str().c_str()); }
  else { klee_warning_once(reason, "%s", os.str().c_str()); }
...
void Executor::executeCall(ExecutionState &state, KInstruction *ki,
                           Function *f, std::vector<ref<Expr>> &arguments,
                           std::vector<ref<Expr>> &returnArgValues, ref<Expr>& returnSymcrete, bool isExtOp) {
  Instruction *i = ki->inst;
  llvm::Optional<std::vector<ref<Expr>>> result = llvm::None;
  ...
  if ((f && f->isDeclaration())) {
    ...
    case Intrinsic::not_intrinsic: {
      ...
      callExternalFunction(state, ki, kmodule->functionMap[f], arguments, returnArgValues, returnSymcrete, isExtOp);
      break;
    }
    case Intrinsic::fabs: {
    ...
    default:
      ...
      return terminateStateOnExecError(state, "unimplemented intrinsic");
    }
...
// The function traverses a vector and applies special handling to any items that are pointers.
std::vector<ref<Expr>> Executor::specialHandleForPointerInArgs(llvm::Function* func,
                                                              ExecutionState& state,
                                                              std::vector<Cell*> argCells,
                                                              std::vector<ref<Expr>> argValues) {
  std::vector<ref<Expr>> preparedArgs;
  preparedArgs.reserve(argValues.size());
  assert(!func->isVarArg() && "vararg is not supported yet");

  for (unsigned i = 0; i < argValues.size(); i++) {
    Type* currentType = func->getFunctionType()->getParamType(i);
    if (currentType->isPointerTy()) { 
      preparedArgs.push_back(argValues[i]); 
    } else {
      preparedArgs.push_back(argCells[i]->value);
    }
  }
...
void Executor::addExtOpConstraint(KInstruction* ki, ExecutionState& state, 
                                  ref<Expr> returnSymcrete, std::vector<ref<Expr>> args) {
  CallInst *ci = cast<CallInst>(ki->inst);
  KFunction* kf = kmodule->functionMap[ci->getCalledFunction()];

  if (kf->getFunctionType()->getReturnType()->isVoidTy()) {
    return;
  }

  auto extop = ExtOpExpr::create(kf, args);
  auto eq = EqExpr::create(returnSymcrete, extop);

  if (RecolossusDebugLog) {
    klee_warning("DEBUG: Add external operation sucessfully (%s)", kf->function->getName().data());
  }

  addConstraint(state, eq);
}

bool Executor::checkKeyExists(const std::string& str, const std::string& key) {
...
bool Executor::isExtOp(const KInstruction *ki, const llvm::Function* f) {
  if (!useRecolossus) return false;
  if (RecolossusExternalFunction == ",") {
    klee_error("The REColossus option is turned on, but no external library functions are set.");
  }
  ...
  std::string file = std::filesystem::path(ki->info->file).filename();
  StringRef name = f->getName();
  std::string line = std::to_string(ki->info->line);

  int w_now = (checkKeyExists(RecolossusRange, file) ? 2 : 0)
            + (checkKeyExists(RecolossusExternalFunction, name.str()) ? 3 : 0)
            + (checkKeyExists(RecolossusLine, line) ? 4 : 0);

  int w_expect = (RecolossusRange != "," ? 2 : 0)
            + (RecolossusExternalFunction != "," ? 3 : 0)
            + (RecolossusLine != "0" ? 4 : 0);
  if (w_now > w_expect) {
    klee_warning("Setting colossus configuration errors");
  }

  if (w_now != w_expect) return false;
  if (f->isVarArg()) {
    klee_error("Variable length parameters are not supported");
    return false;
  } else {
    // klee_warning("Encounter an external function %s (%s %s)", name.data(), file.data(), line.data());
    return true;
  }
}
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      std::ostringstream locInfo;
      auto file = ki->info->file;
      auto line = ki->info->line;
      auto column = ki->info->column;
      locInfo <<  file << ":" << line << ":" << column;
      cond = optimizer.optimizeExpr(cond, false);
      Executor::StatePair branches = fork(state, 
                                          cond, 
                                          false,
  ...
  case Instruction::IndirectBr: {
    ...
    bool result;
    bool success __attribute__((unused)) = solver->mayBeTrue(state.constraints, errorCase, result, state.queryMetaData);
    assert(success && "FIXME: Unhandled solver failure");
    if (result) {
  ...
  case Instruction::Switch: {
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(cond)) {
      ...
      for (std::map<ref<Expr>, BasicBlock *>::iterator
        ...
        bool success = solver->mayBeTrue(state.constraints, match, result, state.queryMetaData);
        assert(success && "FIXME: Unhandled solver failure");
        (void) success;
        if (result) {
      ...
      bool res;
      bool success = solver->mayBeTrue(state.constraints, defaultValue, res, state.queryMetaData);
      assert(success && "FIXME: Unhandled solver failure");
      (void) success;
      if (res) {
  ...
  case Instruction::Call: {
    ...
    //argCells store the expr evaluated from operands of instruction ki, the update of argCells will affect the operands of ki.
    std::vector<Cell*> argCells;
    argCells.reserve(numArgs);
    for (unsigned j=0; j<numArgs; ++j) {
      Cell& cell = (Cell&) eval(ki, j+1, state);
      arguments.push_back(cell.value);
      argCells.push_back(&cell);
    }

    if (auto* asmValue = dyn_cast<InlineAsm>(fp)) { //TODO: move to `executeCall`
      if (ExternalCalls != ExternalCallPolicy::None) {
        KInlineAsm callable(asmValue);
        ref<Expr> returnSymcrete;
        std::vector<ref<Expr>> returnArgValues;
        callExternalFunction(state, ki, &callable, arguments, returnArgValues, returnSymcrete, false); // XXX: Check whether to forbid string.h here
      } else {
    ...
    if (f) {
      ...
      // Deal with `v := extop(args ..)`
      ref<Expr> returnSymcrete;
      std::vector<ref<Expr>> returnArgValues;

      // (%s %s %u)", f->getName().data(),target->info->file.c_str(), target->info->line);
      // fn name memset libc/misc/locale/locale.c 874
      bool flag = isExtOp(ki, f);
      executeCall(state, ki, f, arguments, returnArgValues, returnSymcrete, flag); //argvalues stores constantExpr evaluated from arguments
      if (flag && !returnArgValues.empty()) {
        // XXX: [REColossus] How to deal with environments?
        auto args = specialHandleForPointerInArgs(f, state, argCells, returnArgValues);
        addExtOpConstraint(ki, state, returnSymcrete, args);
      }
    } else {
      ...
      do {
        ...
        ref<ConstantExpr> value;
        bool success = solver->getValue(free->constraints, v, value, free->queryMetaData);
        assert(success && "FIXME: Unhandled solver failure");
        (void) success;
        ...
        if (res.first) {
          ...
          if (it != legalFunctions.end()) {
            ...
            ref<Expr> returnSymcrete;
            std::vector<ref<Expr>> returnArgValues;
            executeCall(*res.first, ki, f, arguments, returnArgValues, returnSymcrete, flag);
            if (flag && !returnArgValues.empty()) {
              // XXX: [REColossus] How to deal with environments?
              auto args = specialHandleForPointerInArgs(f, *res.first, argCells, returnArgValues);
              addExtOpConstraint(ki, *res.first, returnSymcrete, args);
            }
          } else {
            if (!hasInvalid) {
...
void Executor::run(ExecutionState &initialState) {
  ...
  // TODO: [REColossus-later] Support symcrete
  if (usingSeeds) {
...
void Executor::terminateStateOnExit(ExecutionState &state) {
  ...
  // TODO: [REColossus] Fix this
  if (shouldWriteTest(state) || (AlwaysOutputSeeds && seedMap.count(&state))) {
    if (RecolossusDebugLog) { klee_warning("DEBUG: terminateStateOnExit"); }
...
void Executor::terminateStateEarly(ExecutionState &state, const Twine &message,
  ...
  if ((reason <= StateTerminationType::EARLY && shouldWriteTest(state)) ||
    ...
    if (RecolossusDebugLog) { klee_warning("DEBUG: terminateStateEarly"); }
    interpreterHandler->processTestCase(
        state, (message + "\n").str().c_str(),
...
void Executor::terminateStateOnError(ExecutionState &state,
  ...
  if (EmitAllErrors ||
    ...
    if (RecolossusDebugLog) { klee_warning("DEBUG: terminateStateOnError"); }
...
// If REColossus is on, all variables are set to symcrete.
void Executor::callExternalFunction(ExecutionState &state,
  ...
  if (ExternalCalls == ExternalCallPolicy::None && !okExternals.count(callable->getName().str())) {
  ...
  // The argValues store the constantExpr evaluated from arguments
  // And for loop compute the value for each argument
  for (std::vector<ref<Expr> >::iterator ai = arguments.begin(),
    ...
    if (ExternalCalls == ExternalCallPolicy::All) { // don't bother checking uniqueness
      *ai = optimizer.optimizeExpr(*ai, true);
      ...
      // send the current argument with constraint to SMT Solver to get assignment

      auto resultPair = solver->getValueAndSolution(state.constraints, *ai, state.queryMetaData);
      if (!resultPair.hasValue()) {
        assert(0 && "FIXME: Unhandled solver failure");
      }
      // Deconstruction pair and invoke toMemory
      assignment = resultPair->first;
      ce = resultPair->second;
      ...
      if (ObjectPair op;
          ...
          state.addressSpace.resolveOne(ce, op) && !op.second->readOnly) {
        auto *os = state.addressSpace.getWriteable(op.first, op.second);
        os->flushToConcreteStore(solver.get(), state, ExternalCalls == ExternalCallPolicy::All, isExtOp);
      } else {
        // Make all all symbolic parameters of type symcrete (except for pointer)
        if (isExtOp) bindAssignment(assignment, state);
      }
      
      wordIndex += (ce->getWidth() + 63) / 64;
      returnArgValues.push_back(ce);
    } 
    else // uniqueness
    {
      ...
      if (ConstantExpr *ce = dyn_cast<ConstantExpr>(arg)) {
        ...
        if (isa<ConstantExpr>(ce)) {
          ...
          returnArgValues.push_back(nullptr);
        }
      } else {
        terminateStateOnExecError(state,
  ...
  if (!state.addressSpace.copyInConcretes(ExternalCalls == ExternalCallPolicy::All)) {
  ...
  state.addressSpace.copyInConcrete(result.first, result.second, (uint64_t)&error, ExternalCalls == ExternalCallPolicy::All);
#endif

  void* address = (void*)args;
  Type *resultType = target->inst->getType();
  ref<ConstantExpr> ce = ConstantExpr::fromMemory(address, getWidthForLLVMType(resultType));

  if (resultType != Type::getVoidTy(kmodule->module->getContext())) {
    // Here not only should we make symcrete for e, but also for each argument
    if (isExtOp && !returnArgValues.empty()) {
      ref<Expr> e = makeFreshSymcrete(state, ce, target);
      returnSymcrete = e; // Accually symbolic
      bindLocal(target, state, e); // make returned value symcrete
    } else {
      // ce stores the constant value returned by extop
      bindLocal(target, state, ce); // make returned value concrete
    }
  }
}
...
void Executor::executeAlloc(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    while (example->Ugt(ConstantExpr::alloc(128, W))->isTrue()) {
      ...
      bool res;
      bool success =
          solver->mayBeTrue(state.constraints, EqExpr::create(tmp, size), res, state.queryMetaData);
      assert(success && "FIXME: Unhandled solver failure");      
      (void) success;
      if (!res)
        break;
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (unbound) {
    if (incomplete) {
      ...
      // ObjectPair op;
      // bool s = state.addressSpace.resolveOne(ConstantExpr::create(0xFF21A1, Expr::Int64), op);
      address->dump();
      terminateStateOnProgramError(
...
bool Executor::getSymbolicSolution(const ExecutionState &state,
  ...
  bool success;
  klee_warning("Using SMT to compute test cases");
  success = solver->getInitialValues(extendedConstraints, objects, values, state.queryMetaData);
  solver->setTimeout(time::Span());
  if (!success) {
