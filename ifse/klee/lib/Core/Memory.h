...
public:
  ...
  std::vector<ref<Expr>> getAllBytes() const;

  /*
    Looks at all the symbolic bytes of this object, gets a value for them
    from the solver and puts them in the concreteStore.
    When using REColossus, it also sets related arrays to symcretes.
  */
  void flushToConcreteStore(TimingSolver *solver, const ExecutionState &state, bool concretize, bool isExtOp) const;
...
private:
  ...
  //flush range to UpdateList for further reading
  void flushRangeForRead(unsigned rangeBase, unsigned rangeSize) const;
  //flush range to UpdateList for further writing
