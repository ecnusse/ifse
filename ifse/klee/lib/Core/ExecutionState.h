...
private:
  ...
  // Store the concrete values of arrays that have been set symcrete.
  // The symbolic values are not stored because an array itself is a symbol.
  std::shared_ptr<std::map<std::string, ExecutionState::bindings_ty>> symcreteMap;
...
public:
  ...
  llvm::Optional<std::vector<unsigned char>> findSymcreteMap(const Array *array) const;
  void addSymcreteMap(const Array *array, std::vector<unsigned char> concrete) const;

  std::uint32_t getID() const { return id; };
  void setID() { id = nextID++; };
  static std::uint32_t getLastID() { return nextID - 1; };

  std::shared_ptr<std::map<std::string, ExecutionState::bindings_ty>> getSymcreteMap() const;

  llvm::Optional<std::pair<ref<const MemoryObject>, const Array *>>  getArrayFromSymbolic(const std::string& name) const;
