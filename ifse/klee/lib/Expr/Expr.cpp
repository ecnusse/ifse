...
ref<Expr> ConstantExpr::fromMemory(void *address, Width width) {
  ...
  case Expr::Fl80: {
    size_t numWords = (width + llvm::APFloatBase::integerPartWidth - 1) / llvm::APFloatBase::integerPartWidth;
    return ConstantExpr::alloc(llvm::APInt(width, llvm::ArrayRef<uint64_t>((const uint64_t *)address, numWords)));
  }
  case Expr::Int128:
  case Expr::Int256:
  case Expr::Int512: {
...
static SymcreteMapPtr symcreteMapSingleton = nullptr;

void klee::setSymcreteMapSingleton(klee::SymcreteMapPtr symcreteMap) {
  symcreteMapSingleton = symcreteMap;
}

void klee::clearSymcreteMapSingleton() {
  symcreteMapSingleton = nullptr;
}

klee::SymcreteMapPtr klee::getSymcreteMapSingleton() {
  return symcreteMapSingleton;
}

ref<Expr> ReadExpr::create(const UpdateList &ul, ref<Expr> index) {
  auto symcreteMap = getSymcreteMapSingleton();
  if (symcreteMap != nullptr) {
    auto arr = ul.root;
    SymcreteMap::iterator it = symcreteMap->find(arr->name);
    if (it != symcreteMap->end()) {
      UpdateList newUpdateList(ul.root, 0); // Constraints added by the FilteringSolver will guarantee correct values for each index.
  ...
  for (; un; un = un->next.get()) {
    //find from the most recent updateNode to the least recent to find the updateNode that record the lastest update for the byte with index of offset.
...
unsigned ExtOpExpr::computeHash() {
  unsigned res = getKind() * Expr::MAGIC_HASH_CONSTANT;
  res ^= std::hash<const KFunction*>{}(func) * MAGIC_HASH_CONSTANT;
  for (const auto& arg : args) {
    res ^= arg->computeHash() * MAGIC_HASH_CONSTANT;
  }
  hashValue = res;
  return hashValue;
}
