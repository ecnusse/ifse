...
ref<Expr> ExprVisitor::visitActual(const ref<Expr> &e) {
  ...
  {
    // ExprPPrinter::printOne(llvm::errs(), "In visitActual ep", e);
    Expr &ep = *e.get();

    Action res = visitExpr(ep);
    switch(res.kind) {
      case Action::DoChildren: break; // continue with normal action
    ...
    switch(ep.getKind()) {
      ...
      case Expr::ExtOp: res = visitExtOp(static_cast<ExtOpExpr&>(ep)); break;
      case Expr::Constant:
      default: assert(0 && "invalid expression kind");
    }
...
ExprVisitor::Action ExprVisitor::visitExtOp(const ExtOpExpr&) {
  return Action::doChildren(); 
}