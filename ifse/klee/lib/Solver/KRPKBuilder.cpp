...
KRPKNode KRPKNode::make_bvashr(const KRPKNode& lhs, const KRPKNode& rhs) {
    return KRPKNode(krpk_make_bvashr(lhs.node, rhs.node), {lhs, rhs});
}

KRPKNode KRPKNode::make_bvule(const KRPKNode& lhs, const KRPKNode& rhs) {
    return KRPKNode(krpk_make_bvule(lhs.node, rhs.node), {lhs, rhs});
}
...
KRPKNode KRPKNode::make_ite(const KRPKNode& cond, const KRPKNode& then, const KRPKNode& els) {
    return KRPKNode(krpk_make_ite(cond.node, then.node, els.node), {cond, then, els});
}
...
KRPKNode KRPKNode::make_ensure_index(const KRPKNode& constant, const KRPKNode& index) {
    return KRPKNode(krpk_make_ensure_index(constant.node, index.node), {constant, index});
}
...
KRPKNode KRPKNode::make_array_const_with_provided_values(const KRPKNode& sort, const KRPKNode& default_value, const char* provided_values) {
    return KRPKNode(krpk_make_array_const_with_provided_values(sort.node, default_value.node, provided_values), {sort, default_value});
}
...
void KRPKContext::make_alloc_seg(const KRPKNode& addr, const uint64_t size, const char* hex_bytes) {
    auto r = KRPKNode(krpk_make_alloc_seg(addr.node, size, hex_bytes), {addr});
    allocs.push_back(r);
}
}