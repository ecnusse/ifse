...
bool SolverImpl::computeValidity(const Query &query, Solver::Validity &result) {
  ...
  if (!computeTruth(query, isTrue))
    return false;
  if (isTrue) { //implies (not query) is unsatisfiable, so query's validity is True
    result = Solver::True;
  } else { //(not query) is satisfiable, so query can be false or unknown
    if (!computeTruth(query.negateExpr(), isFalse))
      return false;
    //determine if query is unsatisfiable, 
    //if so then query's validity is false
